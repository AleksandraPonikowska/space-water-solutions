...
st.markdown("""
    ...
    [data-testid="stVerticalBlockBorderWrapper"]:has(.trend-box-title) {
        background-color: #EEEBD3;
        border: 1px solid #594F41;
        border-radius: 14px;
        padding: 8px;
        ...
        [data-testid="stHorizontalBlock"] > div:nth-child(1) [data-testid="stVerticalBlockBorderWrapper"] {
        background-color: #EEEBD3 !important;
        border: 1px solid #594F41 !important;
...
sidebar_col, main_col = st.columns([3, 5], gap=None)
...
with sidebar_col:
    with st.container(border=True):
        ...
        if clicked and center_lon is not None and center_lat is not None:
            distances = np.sqrt(
                (chart_data["Lon"] - center_lon) ** 2 +
                (chart_data["Lat"] - center_lat) ** 2
            )
        ...
        with st.container(border=True):
            st.markdown(
                '<div class="trend-box-title">Trend i Predykcja AI</div>',
                unsafe_allow_html=True
            )

            st.scatter_chart(
                dates[dates["Sektor_ID"] == selected_sector],
                x="Data",
                y="NDMI"
            )
